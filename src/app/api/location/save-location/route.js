...
export const POST = withAuth(async function (req, user) {
  await connectDB();
  const { currentCountry, currentCity, destinationCountry, destinationCity } = await req.json();

  if (!currentCountry || !currentCity || !destinationCountry || !destinationCity) {
  ...
  try {
    ...
    return addCorsHeaders(
      NextResponse.json({ message: "Location saved successfully", item: saved }, { status: 200 })
    );
  } catch (err) {
